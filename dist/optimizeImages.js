...
const getRemoteImageURLs_1 = require("./utils/getRemoteImageURLs");
const downloadImagesInBatches_1 = require("./utils/downloadImagesInBatches");
const fs = require("fs");
...
if (process.argv.length === 3) {
    ...
    console.error("next-image-export-optimizer: Breaking change: Please provide the path to the next.config.mjs file as an argument with the name --nextConfigPath.");
    // Reset the color
...
const nextImageExportOptimizer = async function () {
    ...
    const { remoteImageFilenames, remoteImageURLs } = await (0, getRemoteImageURLs_1.getRemoteImageURLs)(nextConfigFolder, folderPathForRemoteImages);
    try {
        ...
        else if (newPath?.nextImageExportOptimizer_storePicturesInAVIF !== undefined) {
            storePicturesInAVIF =
        ...
        else if (newPath?.nextImageExportOptimizer_generateAndUseBlurImages == "true") {
            blurSize = [10];
        }
        ...
        if (nextjsConfig.transpilePackages === undefined || // transpilePackages is not set
            ...
            console.warn("\x1b[41m", `Changed in 1.2.0: You have not set transpilePackages: ["next-image-export-optimizer"] in your next.config.mjs. This may cause problems with next-image-export-optimizer. Please add this line to your next.config.mjs.`, "\x1b[0m");
        }
    }
    catch (e) {
    ...
    if (remoteImageURLs.length > 0)
        console.log(`Found ${remoteImageURLs.length} remote image${remoteImageURLs.length > 1 ? "s" : ""}...`);
    await (0, downloadImagesInBatches_1.downloadImagesInBatches)(remoteImageURLs, remoteImageFilenames, folderPathForRemoteImages, Math.min(remoteImageURLs.length, 20), remoteImageCacheTTL);
    ...
    if (remoteImageURLs.length > 0) {
        ...
        const allFilesInRemoteImageFolder = fs.readdirSync(folderNameForRemoteImages);
    ...
    for (let index = 0; index < allGeneratedImages.length; index++) {
        ...
        const fileInBuildFolder = path.join(exportFolderPath, (() => {
